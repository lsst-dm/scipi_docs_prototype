...
import sys # mg
import lsst.afw.display.ds9 as ds9 # mg
...
class IsrTask(pipeBase.CmdLineTask):
    ...
    def readIsrData(self, dataRef, rawExposure):
        ...
        print " \n******** biasExposure = ", biasExposure # mg
        print " \n********  flatExposure = ", flatExposure # mg
        print " \n******** brighterFatterKernel = ", brighterFatterKernel # mg
#        print " \n********  defectList = ",  defectList # mg - massive obj, we don't print it

        print " \n********  self.config.doFringe = ", self.config.doFringe # mg
    ...
    def runDataRef(self, sensorRef):
        ...
        print " ****************** In isrTask runDataRef" # mg
        print " \n ****** sensorRef = ", sensorRef
                
        self.log.info("Performing ISR on sensor %s" % (sensorRef))
                
        ccdExposure = sensorRef.get('raw')
                
        print " ***** ccdExposure = ", ccdExposure # mg
        # ds9.mtv(ccdExposure) # mg
        
        isrData = self.readIsrData(sensorRef, ccdExposure)
#        print " \n\n ******** isrData = " ,isrData # mg - this is a massively long object, so we don't print it
        ...
        print " self.config.doWrite = " , self.config.doWrite
        return result
